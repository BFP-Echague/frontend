<script lang="ts">
	...
	let clusterCountOptions = $derived(
		finalClusteringResult?.clusterResults.map(
			(clusterResult) =>
				new OptionHelper(
	...
	let maxClusterNumber: number | null = $derived(
		finalClusteringResult !== null
			? Math.max(
					...finalClusteringResult.clusterResults.map((clusterResult) => clusterResult.clusterCount)
				)
			: null
	);
	...
	let selectedClusterCountResult: ClusterResult | null = $derived.by(() => {
		...
		return (
			finalClusteringResult.clusterResults.find(
				(clusterResult) => clusterResult.clusterCount === selectedClusterCount
			) ?? null
		);
	});
	...
	let selectedIncidents: IncidentGet[] | null = $derived.by(() => {
		...
		return finalClusteringResult.incidents.filter((incident) =>
			selectedClusterNumberResult.includes(incident.id)
		);
	});
						...
						<Card class="d-flex flex-column h-100 shadow border">
							...
							<CardBody>
								<div class="d-flex flex-column">
									...
									<DisplayItem
										name="Silhouette Score (Clustering Score)"
										description={selectedClusterCountResult.score.toPrecision(6)}
									/>
									<DisplayItem
										name={`Is optimal (when checking clusters between ${minClusterNumber} and ${maxClusterNumber})`}
										description={selectedClusterCountResult.clusterCount ===
											finalClusteringResult.optimalClusterCount}
									/>
								</div>
								...
								<div class="d-flex flex-column mt-2">
									...
									<DisplayItem
										name="Incident Count"
										description={selectedClusterNumberResult.length}
									/>
								</div>
							</CardBody>
						</Card>
